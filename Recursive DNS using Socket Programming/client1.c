...
void Client1_Server1_Communication(Node *head);
...
int main()
{
    ...
    Client1_Server1_Communication(head);
    return 1;
...
void Client1_Server1_Communication(Node *head)
{
    ...
    /*Code referred from rutgers*/
        ...
        struct sockaddr_in Client1_UDP_Dynamic_Port;
        struct hostent *Client1_IP = gethostbyname("nunki.usc.edu");
        ...
        memcpy(&myaddr.sin_addr, Client1_IP->h_addr_list[0], Client1_IP->h_length);
        ...
        //Upon receiving a valid keyword
         printf("Client 1 has received a request with search word %s, which maps to %s",search_string,searched_key);
        ...
        /*Creation of dynamic port for client 1*/
        len = sizeof(Client1_UDP_Dynamic_Port);
        if(getsockname(fd, (struct sockaddr *)&Client1_UDP_Dynamic_Port, (socklen_t*)&len) == -1)
        {
        ...
        printf("\nThe Client 1 sends the request GET %s to the Server 1 with port number %d and IP address %s.",searched_key,SERVER1_UDP_STATIC_PORT,inet_ntoa(remaddr.sin_addr));
        printf("\nThe Client1's port number is %d and the IP address is %s.",ntohs(Client1_UDP_Dynamic_Port.sin_port),inet_ntoa( *((struct in_addr *)Client1_IP->h_addr) ));
        ...
        recvlen = recvfrom(fd, buf, BUFLEN, 0, (struct sockaddr *)&remaddr, &slen);
            if (recvlen >= 0) 
            {
                ...
                printf("\nThe Client 1 received the value POST %s from the Server 1 with port number %d and IP address %s.",buf,SERVER1_UDP_STATIC_PORT,inet_ntoa(remaddr.sin_addr));
                printf("\nThe Client1's port number is %d and IP address is %s\n",ntohs(Client1_UDP_Dynamic_Port.sin_port),inet_ntoa( *((struct in_addr *)Client1_IP->h_addr) ));
            }

    
        /*Code referred from rutgers*/

        close(fd);  //closing the socket
...
Node* Initialize_Client(Node* head)
{
    ...
    char key[6];
    char value[8];
    ifp = fopen("client1.txt", "r");             //Open client1.txt
